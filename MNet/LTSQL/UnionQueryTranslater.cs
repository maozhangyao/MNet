    ...
    public class UnionQueryTranslater : IQueryTranslater
    {
        public LTSQLToken Translate(QueryPart query, LTSQLTranslateScope scope)
        {
            ...
            foreach (QueryPart sub in set.Querys)
            {
                ...
                if (sub is QuerySetPart && scope.Context.Options.DbType != DbTypes.SQLLite)
                {
            ...
            TupleToken flds = LTSQLTokenFactory.CreateTupleToken(select);
            return LTSQLTokenFactory.CreateSetOperationToken(flds, rts, set.SetType, set.Distinct);
        }
    }
}
