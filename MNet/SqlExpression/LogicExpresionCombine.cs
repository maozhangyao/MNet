...
namespace MNet.SqlExpression
{
    ...
    public class LogicExpresionCombine : ExpressionVisitor
    {
        ...
        {
            return Expression.Parameter(node.Type, this._parameterName);
        }
        public Expression<Func<T,bool>> Combine<T>(params Expression<Func<T, bool>>[] exprs)
        {
            ...
            ParameterExpression p = Expression.Parameter(typeof(T), this._parameterName);
            ...
            return Expression.Lambda<Func<T, bool>>(combine, p);
        }
    }
}
