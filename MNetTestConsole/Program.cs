...
//ConsoleHelper.WriteLineWithYellow(query1.ToSql(DbTypes.SQLLite));
...
//ConsoleHelper.WriteLineWithYellow(query2.ToSql(DbTypes.SQLLite));


//// 联合查询操作(union all)， 由于查询1与查询2字段不一致，所以无法执行查询，仅作sql生成展示
//var query3 = query1.UnionSet(query2, distinct: false);
//Console.WriteLine();
//Console.WriteLine("联合查询操作(union all)：");
//ConsoleHelper.WriteLineWithYellow(query3.ToSql(DbTypes.SQLLite));

var k = new CPersionT();
var sql = LTSQLQueryableExtensions.AsUpdate<CPersionT>(p => new
{
    Id = 1,
    Age = 99
}).Where(p => p.Id == k.AsLTSQL().Where(c => c.Id == p.Id).Select(a => a.Id).FirstOrDefault())
.ToSql(DbTypes.SQLLite);
